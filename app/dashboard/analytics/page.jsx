...
function Analytics() {
  ...
  useEffect(() => {
    ...
    if (email) {
      Promise.all([fetchUserAnalytics(email), fetchInterviews(email)]).then(
        ([answers, interviewList]) => {
          setData(answers);
  ...
  const interviewScores = useMemo(() => {
    ...
    Object.entries(groups).forEach(([id, vals]) => {
      result[id] = {
        avgRating: (
          vals.ratings.reduce((a, b) => a + b, 0) / vals.ratings.length
        ).toFixed(1),
        avgFiller: (
          vals.fillers.reduce((a, b) => a + b, 0) / vals.fillers.length
        ).toFixed(1),
      };
    });
    return result;
  ...
  const stats = useMemo(() => {
    ...
    const trend =
      trendData.length >= 2
        ? (
            trendData[trendData.length - 1].rating - trendData[0].rating
          ).toFixed(1)
        : null;
    ...
    <div className="p-6 md:p-10 max-w-6xl mx-auto">
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          ...
          <Card className="p-5 bg-slate-900 text-white">
            ...
            <p
              className={`text-3xl font-bold ${parseFloat(stats.latest) >= 7 ? "text-green-400" : "text-amber-400"}`}
            >
              {stats.latest}
          ...
          <Card className="p-5 bg-slate-900 text-white">
            ...
            <p
              className={`text-3xl font-bold ${parseFloat(stats.trend) >= 0 ? "text-green-400" : "text-red-400"}`}
            >
              {stats.trend !== null
      ...
      <section className="mb-12">
        ...
        <p className="text-sm text-slate-500 mb-6">
          Each point = one interview session average
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <Card>
            <CardHeader>
              <CardTitle>Score Per Interview</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Filler Words Per Interview</CardTitle>
            </CardHeader>
            <CardContent className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {interviews.map((iv) => {
              ...
              return (
                <Card
                  key={iv.mockId}
                  className="flex flex-col justify-between p-5 hover:shadow-md transition-shadow"
                >
                  <div className="mb-4">
                    ...
                    <p className="text-xs text-slate-500">
                      {iv.jobExperience} yrs exp • {iv.createdAt}
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xl font-bold ${rating >= 7 ? "text-green-600" : rating >= 5 ? "text-amber-500" : "text-red-500"}`}
                    >
                      {score?.avgRating ?? "—"}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        router.push(
                          `/dashboard/interview/${iv.mockId}/feedback`,
                        )
                      }
                    >
                      View Report
